...
# torres y muros.
class Torre:
    # Diccionario con los tipos de torres disponibles
    TIPOS = {
        "olimpo": {
    ...
    def __init__(self, tipo):
        # Inicializa una torre según el tipo seleccionado, por ende carga los atributos
        tipo = tipo.lower()
    ...
    def calcular_costo(self):
        # Retorna el costo de compra de la torre
        if self.tipo == "oscura":
            return 35
...
class Muro:
    #biblioteca con los muros y sus info
    TIPOS = {
        "madera": {
    ...
    def calcular_costo(self):
        return self.resistencia_max // 2        self.turnos_habilidad = datos["turnos_habilidad"]
        self.costo = self.calcular_costo()
        self.contador_habilidad = 0
        self.congelada = 0
        self.debilitada = False
    def calcular_costo(self):
        costo = (self.dano * 5) + (self.alcance * 10) + (self.vida_max // 10)
        if self.tipo == "olimpo":
            costo += 20
        elif self.tipo == "oscura":
            costo += 15
