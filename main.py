...
# menu de juego cuando entra el primero y segundo
def vtn_menu_juego(principal, jugador):
    ...
    canvas.create_text(
        500,
        200,
        text=f"Bienvenid@, {jugador.username}!",
        fill="white",
        font=("Copperplate", 30, "bold")
    )
...
def vtn_segunda_sesion(menu_win, jugador1, faccion1):
    ...
    def Zclick(x1, y1, x2, y2, funcion):
        ...
        canvas.tag_bind(zona, "<Button-1>",
                        lambda e: funcion())
        canvas.tag_bind(zona, "<Enter>",
                        lambda e: canvas.config(cursor="hand2"))
        canvas.tag_bind(zona, "<Leave>",
                        lambda e: canvas.config(cursor=""))
...
def vtn_partida(padre, jugador1, jugador2, faccion1, faccion2):
    ...
    lbl_log.pack(pady=8)
    btn_accion = tk.Button(frame_info, font=("Arial", 10), width=20)
